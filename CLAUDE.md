...
- [x] **Paragraph editing with reflow** — clicking a multi-line block with Edit Text opens the whole paragraph in a WYSIWYG editor; on commit PDFium removes the original runs and re-inserts the text greedy-wrapped to the paragraph width at its original leading/alignment/colour (`getParagraphAt`/`replaceParagraphAt` in pdfiumEngine.ts; proof: `scripts/prove-paragraph-edit.mjs`). A complete installed system font matching the document face is preferred over subsetted embedded fonts so new characters have glyphs. Single-line paragraphs keep the in-place path (exact font preserved)
