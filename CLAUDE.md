...
- [x] **PDF → Excel (.xlsx)** — detect → review → export flow in the Export dialog (`src/renderer/utils/extractTables.ts`). Rows clustered by baseline (adaptive tolerance), columns from whitespace-gap projection across rows; numeric cells stored as real numbers. Four reading engines: **Automatic** (native text; scanned pages auto-OCR'd with Tesseract + ruled-line column detection from rendered pixels), **Force OCR**, **Local handwriting** (TrOCR q8 via `@huggingface/transformers` in the main process, `src/main/trocrEngine.ts`; two model sizes selectable in the dialog and persisted as `settings.trocrModel` — **base** `Xenova/trocr-base-handwritten` ≈340 MB, the default, better digits and far fewer hallucinations, vs **small** `Xenova/trocr-small-handwritten` ≈80 MB, faster; models cached side by side in userData/trocr-cache, only one resident in memory at a time, fully offline after first download; pages are segmented into cell crops by `segmentTableCells` — binarize → strip long straight runs → connected-component filtering of dashed/curved rule fragments → row bands from ink projection → per-row runs voted into columns, falling back to per-row gap splitting when no global column grid exists; crops are recognised from the CLEANED ink bitmap so ruled lines never reach the model), and **Azure AI** (Document Intelligence prebuilt-layout via `azure:layoutAnalyze` IPC — best handwriting accuracy, returns table cells directly). An editable review grid (contentEditable cells, per-page tabs) lets the user fix misreads before saving. **Styled output (Acrobat parity)**: the workbook is written with exceljs (`src/renderer/utils/styledExcel.ts`) — fonts/sizes/bold/italic/text colour come from PDFium styled runs (`getStyledTextRuns` resolves real family names + weights out of the embedded font programs via fontkit, since PDF base names are often anonymised `CIDFont+F1`), cell fills and per-side borders are sampled from rendered page pixels (border scan reaches halfway to the neighbouring row/column), spanning headings become merged centred ranges, `15,900`→`#,##0` / `47.00`→`0.00` number formats are preserved, column widths follow the PDF column extents, and a "Combine all pages into one continuous sheet" option produces a single `Table 1` like Acrobat. Proofs: `scripts/prove-excel-export.mjs`, `scripts/prove-trocr-excel.mjs`, `scripts/prove-styled-excel.mjs`. Plain Tesseract cannot read handwriting; TrOCR reads it locally with review-grid-level accuracy, Azure reads it best.
...
| **Excel export** | Export → Excel tab → pick engine (Automatic / Force OCR / Local handwriting / Azure AI) → Detect Tables → fix any cell in the review grid → Export to Excel → open in Excel: one sheet per page, numbers as numbers |
| **Excel from scan** | Open a scanned PDF → Export → Excel → Automatic → Detect → OCR runs, grid appears; for handwriting use Local handwriting (offline; Best quality ≈340 MB or Fast ≈80 MB one-time download, picker under the engine card) or Azure AI (endpoint+key in Settings → API keys) |
| **API keys** | Ctrl+, → API keys: type Anthropic / Azure key → Apply → reopen: input is empty with `••••` placeholder (key never shown again); type new key to replace, Remove to clear; Azure endpoint field below; settings Export JSON excludes all secrets |
